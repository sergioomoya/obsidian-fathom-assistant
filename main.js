/* Fathom Assistant Plugin - Compilado por ESBuild */
...
__export(main_exports, {
  FathomChatView: () => FathomChatView,
  VIEW_TYPE_FATHOM_CHAT: () => VIEW_TYPE_FATHOM_CHAT,
  ...
  getGeminiFunctionDeclarations() {
    ...
    for (const [serverName, session] of this.sessions.entries()) {
      for (const tool of session.tools) {
        ...
        const rawProperties = (_a2 = tool.inputSchema) == null ? void 0 : _a2.properties;
        ...
        if (hasProperties) {
          ...
          const rawRequired = (_b = tool.inputSchema) == null ? void 0 : _b.required;
          const required = Array.isArray(rawRequired) ? rawRequired.filter((r) => typeof r === "string" && properties[r]) : void 0;
          parameters = {
            type: Type.OBJECT,
            properties,
    ...
    if (mappedType === Type.ARRAY) {
      ...
      if (schema.properties && typeof schema.properties === "object" && Object.keys(schema.properties).length > 0) {
        ...
        if (Array.isArray(schema.required) && schema.required.length > 0) {
          result.required = schema.required.filter((r) => typeof r === "string" && result.properties[r]);
        }
      }
    }
    if (Array.isArray(schema.enum) && schema.enum.length > 0) {
      result.enum = schema.enum.map((e) => String(e));
    }
    return result;
  }
  mapSchemaType(typeVal) {
    if (Array.isArray(typeVal)) {
      const primary = typeVal.find((t) => t !== "null") || "string";
...
var FathomChatView = class extends import_obsidian2.ItemView {
  ...
  getViewType() {
    return VIEW_TYPE_FATHOM_CHAT;
  }
  getDisplayText() {
    return "Fathom Assistant";
  }
  getIcon() {
    return "bot";
  }
  async onOpen() {
    ...
    this.titleInputEl = header.createEl("input", { type: "text", cls: "chat-title", value: "Nueva Conversaci\xF3n" });
    ...
    const optionAttach = popoverList.createEl("li", { text: "\u{1F4CE} Adjuntar Archivo" });
    const optionMention = popoverList.createEl("li", { text: "@ A\xF1adir Contexto", cls: "disabled", title: "Pr\xF3ximamente" });
    const attachBtn = chatFooter.createEl("button", { cls: "fathom-attach-btn", title: "A\xF1adir Contexto" });
...
`;
    ...
    if (!this.currentChatFile) {
      const folder = await this.getChatsFolder();
      let title = this.titleInputEl.value || "Nueva Conversaci\xF3n";
      if (title === "Nueva Conversaci\xF3n") {
        const firstMsg = (_a2 = this.chatHistory[0]) == null ? void 0 : _a2.text;
            ...
            item.setTitle("A\xF1adir contexto a Fathom Assistant").setIcon("bot").onClick(() => {
              ...
              if (leaves.length > 0) {
                ...
                this.activateView().then(() => {
                  ...
                  if (lvs.length > 0) {
                    lvs[0].view.addContextItem(file);
                  }
                });
              }
            });
